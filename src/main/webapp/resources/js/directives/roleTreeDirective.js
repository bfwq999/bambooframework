angular.module("myApp.roleTreeDirective", [])
.directive("myRoletree", ["Restangular",function(Restangular) {
	return {
		restrict: 'A',
		require: '?ngModel',
		replace:true,
		template:function(tElement,tAttrs){
			...
			<ul id="'+menuId+'" class="dropdown-menu menu" role="menu" ng-show="isPermitted(\'role:add\') || isPermitted(\'role:edit\') || isPermitted(\'role:delete\')">\
				<li role="presentation" act="edit" ng-show="isPermitted(\'role:edit\')"><a role="menuitem" tabindex="-1" href="#">编辑角色</a></li>\
				<li role="presentation" act="del" ng-show="isPermitted(\'role:delete\')"><a role="menuitem" tabindex="-1" href="#">删除角色</a></li>'
			+'</ul></div>'; 
			return menuHtml;
		},
		compile:function(tElement,tAttrs,transclude){
			return function($scope, $element, $attrs, $ngModel) {
				...
				$scope.$watch("isPermitted",function(newValue){
					...
					setting.edit.enable = $scope.isPermitted("role:edit");

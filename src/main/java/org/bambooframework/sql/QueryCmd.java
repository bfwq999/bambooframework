...
import org.bambooframework.sql.parse.query.QueryParam;
import org.bambooframework.sql.parse.query.QueryResultTable;
import org.bambooframework.sql.parse.query.QueryResultView;

public class QueryCmd implements Command {
	protected QueryParam dataMap;
	protected Database database;
	protected int colIndex = 0;

	public QueryCmd(QueryParam dataMap) {
		super();
	...
	public String parserView(QueryResultView view){
		...
		view.getResult()
		/*
		for(Map.Entry<String, Object> entry:view.getColumns().entrySet()){
			if(returnSql.length()>0){
				returnSql.append(",");
			}
			String alias = entry.getKey();
			Object value = entry.getValue();
			if(value instanceof QueryResultView){
				returnSql.append("(")
					.append(parserView((QueryResultView) value))
					.append(")")
					.append(" ").append(alias);
			}else if(value instanceof QueryResultTable.OneToManyTable){
				
			}
		}*/
		
		for(QueryResultView.JoinTable join :view.getJoins()){
			
		}
		return null;
	}
	
	public String[] parserTable(QueryResultTable table){
		StringBuffer returnSql = new StringBuffer();
		StringBuffer fromSql = new StringBuffer();
		StringBuffer whereSql = new StringBuffer();
		if(table instanceof QueryResultView){
			returnSql.append("(")
			.append(parserView((QueryResultView) table))
			.append(") ").append(table.getAlias());
		}else{
			String[] sql = parserColumn(table.getColumns());
	 ...
	 * @Description:
	 */
	public String[] parserColumn(Map<String, Object> colData){
		StringBuffer returnSql = new StringBuffer();
		StringBuffer fromSql = new StringBuffer();
		StringBuffer whereSql = new StringBuffer();
		StringBuffer orderBySql = new StringBuffer();
		for(Map.Entry<String, Object> entry:colData.entrySet()){
			if(returnSql.length()>0){
				returnSql.append(",");
			}
			String alias = entry.getKey();
			...
			if(value instanceof QueryResultView){
				...
				QueryResultTable.OneToManyTable oneToManyTable = (QueryResultTable.OneToManyTable)value;
				String[] sqls = parserTable(oneToManyTable.getTable());
				//找到外键关联字段
				Table table1 = DatabaseUtils.getTable(oneToManyTable.getParent().getName());
				Table table2 = DatabaseUtils.getTable(oneToManyTable.getTable().getName());
				Column col1 = table1.getPrimaryKey();
				Column col2 = table2.getForeignColumn(table1.getPrimaryKey());
				returnSql.append(sqls[0]); //返回列
				fromSql.append(" left join ").append(sqls[1])
					.append(" on ")
					.append(oneToManyTable.getParent().getAlias()).append(".").append(col1.getName())
					.append( " = ")
					.append(oneToManyTable.getTable().getAlias()).append(".").append(col2.getName())
					.append(" ").append(sqls[2]);// from on
				orderBySql.append(sqls[3]); //排序
			}else{
				returnSql.append(value.toString());
			}
		}
		return new String[]{returnSql.toString(),fromSql.toString(),orderBySql.toString()};
	}
	
	public String[] parserJoin(QueryResultView view,QueryResultView.JoinTable join){
		StringBuffer returnSql = new StringBuffer();
		StringBuffer fromSql = nedw StringBuffer();
	...
	public String buildSql(){
		QueryResultView view = dataMap.getQuery();
		StringBuffer sb = parserView(view);
		return null;
	}
	...
	private void loadSimpleData(ResultSet rs, ResultTable tbl) throws SQLException {
		if (tbl.oneToManys().size() == 0 && tbl.getForeignTables().size()==0) {
			...
			if(paths.size() == 1){
				...
				tblDataMap.put(tbl.getAlias(), data);
				dataMap.add(tblDataMap);
				...
				for(ResultTable rt : paths ){
					...
					if(rootDataMap instanceof List){
						...
						if(obj == null){
							...
							if(rt.parent.isForeign(rt.getId())){
